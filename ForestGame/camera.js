...
function forward() {
    ...
    let viewDirection = normalize(subtractV3V3(at, eye));
    let newEye = scaleAndAdd(viewDirection, eye);
    //Check new eye position is inbounds before moving
    if (newEye[0] < -X_BOUND || newEye[0] > X_BOUND ||
        newEye[2] < -Z_BOUND || newEye[2] > Z_BOUND) {
        return;
    }
    let newAt = scaleAndAdd(viewDirection, at);
...
//Add normalized vector a * MOVE_STEP to b
//with MOVE_STEP determining degree of movement
function scaleAndAdd(a, b) {
    let out = vec3();
